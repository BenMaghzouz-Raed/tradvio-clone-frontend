...
import ManageSubscriptionPlans from "./components/subscription-plan";
...
export default function AdminPanel() {
  ...
  const stats = useMemo(() => {
    ...
    const newUsers = users.filter(
      (u) => new Date(u.created_at) >= last7d
    ).length;
    const activeSubs = subs.filter(
      (s) => String(s.status).toLowerCase() === "active"
    ).length;
    const auditLast7d = logs.filter(
      (l) => new Date(l.created_at) >= last7d
    ).length;
    ...
    <main className="space-y-6">
      <div className="flex flex-wrap justify-around gap-4 mb-4">
        ...
        <StatsCard
          ...
          value={String(stats.auditLast7d)}
